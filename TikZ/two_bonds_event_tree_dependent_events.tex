...
\begin{document}
	\begin{tikzpicture}[
		...
		\draw (B1) -- (C1) node[midway, left=8pt] {$P(N|N) = 0.956$};
		\draw (B1) -- (C2) node[midway, right=8pt] {$P(D|N) = 0.044$};
		\draw (B2) -- (C3) node[midway, left=8pt] {$P(N|D) = 0.4$};
		\draw (B2) -- (C4) node[midway, right=8pt] {$P(D|D) = 0.6$};
		
		% Payoffs below the nodes at t=1
		\node[below=8pt] at (C1.south) {$B_{NN} = \begin{pmatrix} 1 \\ 1 \end{pmatrix}$};
		\node[below=8pt] at (C2.south) {$B_{ND} = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$};
		\node[below=8pt] at (C3.south) {$B_{DN} = \begin{pmatrix} 0 \\ 1 \end{pmatrix}$};
		\node[below=8pt] at (C4.south) {$B_{DD} = \begin{pmatrix} 0 \\ 0 \end{pmatrix}$};
		
		% Solid black line for pooling and tranching
		\draw[thick] (-5, -7.5) -- (5, -7.5);
		
		% New payoff vectors below the solid line
		\node[below=40pt] at (C1.south) {$P_{NN} = \begin{pmatrix} 1 \\ 1 \end{pmatrix}$};
		\node[below=40pt] at (C2.south) {$P_{ND} = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$};
		\node[below=40pt] at (C3.south) {$P_{DN} = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$};
		\node[below=40pt] at (C4.south) {$P_{DD} = \begin{pmatrix} 0 \\ 0 \end{pmatrix}$};
