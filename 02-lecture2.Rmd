...
Feller (@Feller1968) in his famous probability textbook, these numbers are of the same
...
alos a few counties which get value "X". We also want to filter out those. Let's use
...
distribution in a real financial data-set and compare to Benford's law.

## Summary

## Project 2: Should we trust European supervisory bank data?

The European Banking Authority EBA conducts a biannual stress testing excercise for the
...
IFRS9_Stages variable we have to require that it is in `c(1,2,3)`. Al other Variables Exposure, Status, CR_guarantees and CR_exp_moratoria are set to 0. Finally we only look at non zero Amounts.
