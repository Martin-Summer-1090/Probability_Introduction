...
of R in this lecture to build more complex simulations as well as
...
We also learned some basic uses of R and how we can use R to build
...
are now going to enhance and deepen.
...
We pin down the possible outcomes by agreeing *at the outset* what
we want to consider as the possible outcomes.
Take the simple example of considering whether the price of a stock is
going to rise or fall at the next day. In practical situations the outcome
...
::: {.callout-tip #outcome-event-simple-event}
...
**Example:** For 
${\cal S} = \{\text{rise},\text{fall} \}$, the event $\{\text{rise}\}$ is a simple event, while the 
...
The function has to fulfill three properties:
...
I have introduced here probabilities with relation
...
denoted $E_0$, may also be considered. In this case, when the basic
...
we will
also discuss some basic non-discrete sample spaces later in the lectures. 
...
Lets say we want to get the complement, or the set difference of $A \cup B$ 
with respect to the
sample space ${\cal S}$.

The R implementation of the set difference operation is the function
`setdiff()`. This is how we would tell R to compute the set difference
of the union of $A$ and $B$ and the sample space ${\cal S}$ 
in our example
of the die:
...
This complement is the dice shown in the light reddish area, i.e. all the elements of ${\cal S}$
which are not in $A \cup B$.

::: {.callout-tip #mutually-exclusive}
...
$P(A \cup B) = P(A) + P(B)$. We can add up the probabilities because when events
...
An LLM can be a helpful tool to explore concepts 
...
relative frequencies of probabilities in repetitions of
a random experiment.
...
As the number of independent and identically distributed (i.i.d.) trials 
increases, the relative frequency of an event converges to its true 
probability with high probability.
:::
...
Let's go back to the example of the fair coin we played with in
...
and adjusted closing prices^[The **adjusted closing price** of a stock is a modified version of the closing price that reflects the effects of any corporate actions, such as stock splits, dividends, and rights offerings. 
It provides a more accurate representation of the stock's value over time, especially 
when analyzing historical data or calculating investment returns.]
for each day starting in January 1990.
...
by using `coin <- c("H", "T")` instead of `coin <- c(1,0)`. 
...
This transforms the vector into a matrix with 3 rows and 1 column.
...
of many R objects. However, real-world data often require more 
...
Suppose we take our original data about the Apple stock price and remove
the `up_day` column, we had created before:
...
To identify days when the price decreased, we can compute the daily price change 
and create a factor to categorize the changes (`"up"`, `"down"`, `"unchanged"`). 
Then, we can use factors to tabulate occurrences for each day of the week.
...
Then we can use our table to see how many down moves we count on Mondays
in our data set.
...
This analysis reveals how many times Apple’s stock price decreased on Mondays 
in the dataset. Using factors, we can categorize data and extract meaningful 
insights, making them an invaluable tool for financial analysis and beyond.
They are a powerful tool in exploratory data analysis involving
categorical data.
...
1. **Extract Weekly Data:** The `head` function selects the first five rows from `aapl_prices` to simulate a single trading week.
2. **Add Days as Factors:** A new column `day` assigns ordered factors representing the days of the week.
3. **Simulate Price Movements:** Random price movements (`-1`, `0`, `1`) are generated using `sample`, with probabilities for down, unchanged, and up movements.
...
aux_2 <- aapl_prices[1:nrow(aapl_prices), "close"]
...
### Example: Benford's Law and Trading Volumes
...
One of the most powerful ways to use a **Large Language Model (LLM)** is as a **debugging companion** and a tool for 
