...
In order to do that, suppose you participate in an investment game. You get a starting wealth of
...
The game is divided into 20 separate rounds, each consisting of 50 coin flips. At the
...
R to make a green coin and flip it, using the sample function:
```{r make-green-coin}
green_coin <- c("H", "T")
```
Now comes the function which actually implements the coin flip:
```{r green-flip}
...
Now let's do a round of 50 flips:
```{r flipping-green-50}
...
length(sel_heads)
```
This means we have 33 cases where we get 2 and 27 cases where we get 0. Thus on average
we will get:
```{r average-payoff-green-coin}
(72/100)*2
```
So if we payed 1.20 for that round we would make a profit of 0.24. We will discuss the theory
behind this reasoning in more detail in the next lecture. This is called an expected value.

You see the idea now. If the coin is green we should pay for the flip at most
```{r}
0.7*2 + 0.3*0
```
and if its red
...
coin. This we have to guess. Maybe we have some evidence we can use to inform such a guess. Assume
...
probability is 0.9 that the randomly draw coin is green. Now combining this belief with the
...
agin up tails and you loose again 1.20. Now eight more flips come and each time you
...
1. The referee drew a red coin. Possible but unlikely, because you have investigated to bucket and found a ratio of 9 to 1 green over red coins.
...
```
In the figure, you see that I have connected the two nodes $G \cap H$ and $R \cap H$ bya dashed 
...
it comes up H or T. Thus the two nodes connected by the dahsed line are informationally
indistinguishable for you as a participant.
...
We will leave the discussion about investing and updating here. In the project for this
lecture, I will ask you to use R to simulate the investment game and compare results when
you do Bayesian updating to support your bidding compared with other strategies.
