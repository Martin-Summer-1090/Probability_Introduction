...
Now what is this good for? Assume we have an investment strategy, which tells us that we invest
$1/4$ of our wealth in asset 1, and $3/8$ in asset 2 and 3.  
Given we have an initial wealth of $1000$ this would
mean we invest `1000/4` or `r 1000/4` in asset 1 and `r 3000/8` in asset 2 and 3.

This is convenient, because we can now compute the total portfolio payoff by simple
matrix multiplication in one operation. For those of you who have 
forgotten how to multiply matrices or how to multiply matrices and vectors, look at this 
visual explanation as a refresher.

Suppose we have a $m \times n$ matrix $A$ and an $n \times 1$ column vector $p$. We would
like to compute:
\begin{equation*}
A \cdot p
\end{equation*}
Let $A$ our asset matrix and $p$ our vector of portfolio weights, say $p = (1/4, 3/8, 3/8)$, then
```{r matrix-multiplication, out.width='90%', fig.align='center', fig.cap='Multiplying a matrix with a column vector. Take the inner product (i.e. multiply components and add up) with each row of the matrix, ', echo = F}
knitr::include_graphics('figures/matrix_multiplication.png')
```

The R operator of matrix multiplication is `%*%` implements this operation. Thus
let us define:
```{r portfolio-defined}
portfolio <- c(1/4, 3/8, 3/8)*1000
```
Then we can compute the payoffs by:
```{r portfolio-payoffs}
payoff <- A%*% portfolio
payoff
```

R has added up the total column payoffs and kept the row names.

Now we have a strategy to compute portfolio returns in one go as well. Before we do that, let
us make a simple observation about portfolio returns. Assume we have portfolio weights $w^i$. The
fraction of total wealth invested in asset $i$ at $t=0$ can then be expressed as
\begin{equation*}
X_0^i = w^i X_0 \quad i = 1, \cdots, n$
\end{equation*}
Now let $R^i$ be the total return on asset $i$, then $R^i X_0^i = R^i w^i X_0$. The total
amount received at the end of the period is the sum over all these individual amounts, hence
the total return of the portfolio is
\begin{equation*}
R = \frac{\sum_{i=1}^n R^i w^i X_0}{X_0}
\end{equation*}
Equivalently, since $\sum_{i=1}^{n} w^i = 1$, we have
\begin{equation*}
r = \sum_{i=1}^n w^i r^i
\end{equation*}
