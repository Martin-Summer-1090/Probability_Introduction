...
The probability of no-collision ($nc$) with $n$ hashed-messages through a SHA-256 hash function is
\begin{equation*}
P_{nc}(n) = 1 \times \left(1 - \frac{1}{2^{256}}\right)\times \left(1 - \frac{2}{2^{256}}\right) \times \cdots \times \left(1 - \frac{(n-1)}{2^{256}}\right)
\end{equation*}
...
\times \exp\left(-\frac{(n-1)}{2^{256}}\right) \\
 ...
 &\approx \exp\left(-\frac{n^2}{2 \times 2^{256}}\right) \\
 &\approx \exp \left(- \frac{1}{2} \left(\frac{n}{2^{128}}\right)^2 \right)
\end{align*}
Now if $n \ll 2^{256/2}$ and thus $P_c = (1 - P_{nc}) \ll 1$ we can again use the approximation
$\exp(x) \approx 1 + x$ to get the probability of a hash-collision as
\begin{equation*}
P_c(n) \approx \frac{1}{2} \left( \frac{n}{2^{128}} \right)^2
\end{equation*}
Now as long as the number $n$ of hashes is significantly less than $2^128$ this probability
is so small as to be practically negligible.

Assume, for instance, the desired collision probability threshold should be $0.01 \%$. Then
we have:
```{r collision-probability-sha-256}
2^128*sqrt(2*0.0001)
```
This is a pretty huge number. For a comparison of the orders of magnitude, at the time
of writing up these notes (November 30 2021), the age of the universe in seconds was
`4.361171e+17` seconds. Thus the number of hashes that can be performed by SHA-253 before
a collision occurs, is about a factor of $10^19$ larger than the entire seconds passed since
...
You have learned about how to think in probability about collision probabilities and
how to solve for them analytically, by simulation and for large numbers by approximation. 
We have convinced ourselves that the cryptographic Hash-function SHA-256, while it can
produce collisions in theory, practically the number of hashes to make such a collision occur
would be so large that for practical reasons we can be confident that hashing bit stringe with
...
After every lecture unit we formulate a project. A project is a kind of exercise
where you can apply the concepts we have learned in this lecture and thereby 
deepen your knowledge and anchor the newly learned concepts in your long term 
memory.

The first project is about one of the very few dice games offered at a Casino: Craps.
In Craps people throw a pair of dice. We will come back to this game time and again
during this course. In the first project we confine ourselves to model the throwing
of two dice and summing up theory points on the computer. We try to find out the 
...
picture change?
5. Now look at the random experiment of throwing two dice with the concepts from
...
6. In the early 17th century the Grand Duke of Tuscany was puzzled because he thought that
...
can this be ?", the count asked Galileo Galilei who composed a short note to explain to the
