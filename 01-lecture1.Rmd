...
in a gray box, as before but without the ```>``` prompt. When you move the cursor on the gray box
...
on strings. We will see many examples of this later. Strings are always between ```" "```, like this. Like
numbers, strings can be stored in objects and referred to later. But what we need here is actually
two strings in one vector. This is achieven by using the `c()` function of R. c stands for concatenation
and here it stores both basic outcomes of a coin throw in two single vectors `coin1` and `coin2`.

We can now pass these two virtual coins to the R function `expand.grid()`. This function will produce
...
in probability: The random placement of `r`` balls in `n` cells. The mathematical notation for such
a counting problem is:
$\binom{n}{r} = \frac{n!}{k!(n - r)!}$
The left habd side of the equation means "the number of ways to choose $r$ things from among $n$ things.
The right hand side shows a formular how this can be computed. R has a function to compute these things
directly. It is called `choose()` and we apply it to the birthdays example
