...
Let's revisit our example represented in @fig-pooling-and-tranching-dependent. 

Each branch represents a possible state of the restructured bond portfolio, 
denoted as $R_{NN}$, $R_{ND}$, $R_{DN}$, and $R_{DD}$:
- $R_{NN}$: Neither bond defaults.
- $R_{ND}$: Bond $1$ does not default, but bond $2$ defaults.
- $R_{DN}$: Bond $1$ defaults, but bond $2$ does not default.
- $R_{DD}$: Both bonds default.

Here is the complete, revised, and **consistent passage**, addressing all inconsistencies while aligning it with the updated notation for bond types ($B_1$ and $B_2$) and conditional/joint probabilities:
...
P_N_given_N <- 0.86/0.9  # Probability that B_2 does not default given B_1 does not default
P_D_given_N <- 0.04/0.9  # Probability that B_2 defaults given B_1 does not default
...
P_N_total <- P_N
P_D_total <- P_D
P_N_given_total <- P_NN + P_DN
P_D_given_total <- P_ND + P_DD
...
### Symmetry of Conditional Probabilities

Using the contingency table, we compute conditional probabilities and highlight their asymmetry:

```{r}
# Conditional probability of B_2 defaulting given B_1 defaults
P_D_given_D <- P_DD / P_D_total

# Conditional probability of B_1 defaulting given B_2 defaults
P_B1_given_B2 <- P_DD / P_D_given_total

cat("P(B_2 = D | B_1 = D):", P_D_given_D, "\n")
cat("P(B_1 = D | B_2 = D):", P_B1_given_B2, "\n")
```
...
This asymmetry reflects the inherent directionality in conditional probability.
...
```{r where-is-the-speck-now-small-overlap, out.width='50%', fig.align='center', fig.cap='Small overlap between $A$ and $B$', echo = F}
knitr::include_graphics('figures/small_overlap.png')
```

- If the overlap is large, $P(B|A)$ will be large:

```{r where-is-the-speck-now-big-overlap, out.width='50%', fig.align='center', fig.cap='Large overlap between $A$ and $B$', echo = F}
