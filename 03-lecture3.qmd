...
To understand the real world aspects of this example it is necessary
...
think of these state realizations of the bonds as occurring **simultaneously**. 
Think of it as if you would toss two coins at once.

- At the edges we write the probability of the outcomes. For example $P(N)$ is
the probability of the first bond (represented by the upper tree) not
defaulting whereas $P(D)$ denotes the probability of the bond defaulting.
...
in analogy to the toss of two fair coins. The probability of the second
...
Remember the complementarity rule? This says that the second restructured 
bond will thus have 
a default probability of $19$ % instead of $10%$ it would be 
speculative grade or close to toxic junk now.

Here is picture how you can visualize this piece of financial magic.
...
bonds by $B$. Underneath these original bonds we draw a black horizontal line,
...
We first set up the parameters as in our tree or as in the
contingency table.
...
Now we simulate joint outcomes based on our
contingency table:
...
This sets the ground for generating the portfolio data in a special
dataframe:
...
Now we can compute the unconditional and the conditional probabilties from
our simulated data:
...
Note that computing conditional probabilities involves focusing on a subset of the data where a specific condition holds. For example, to compute P(X = D | Y = D),
we calculate the relative frequency of X_Defaulted being TRUE within the subset
of rows where Y_Defaulted is TRUE.
...
Let's imagine how you could have assessed the market situation by looking
at financial data. This is also an excellent opportunity to introduce you
...
One thing an analyst might be interested in is how the stock market as measured
...
the Fed St. Louis.^[See https://fred.stlouisfed.org/. These data could in principle also be retrieved directly using tidyquant's `tq_get()` function with the argument `get = "economic.data"`. For these notes, however, I decided to download the data directly from the FRED website and read them from a local folder for robustness reasons, as API-based retrieval can sometimes fail due to network issues or changes in the data provider's interface.]

```{r retrieving-spfundamentals}

# Read corporate profits data from local FRED download
cp_file_path <- "data/Corporate_profits_after_tax_(without_IVS_and_CCAdj)_FRED_Jan_18_2026.csv"
corporate_profits_full <- read.csv(cp_file_path, stringsAsFactors = FALSE)

# Rename columns to match expected format (FRED CSVs typically have DATE and value columns)
names(corporate_profits_full) <- c("date", "price")

# Convert date column to Date class
corporate_profits_full$date <- as.Date(corporate_profits_full$date, format = "%Y-%m-%d")

# Filter for 2000-2011
corporate_profits <- subset(corporate_profits_full, 
...
# Filter corporate profits data for 2000-2019 (using data loaded earlier)
corporate_profits <- subset(corporate_profits_full, 
                            date >= as.Date("2000-01-01") & 
...
#### The Need for Updating Probability Assessments
...
## Project: The Coin-Flipping Investment Game: Bayesian Updating and the Cost of Overconfidence

### Introduction and Motivation

This project is inspired by a brilliant analogy from the world of investing. Consider an 
investor in 2011 who believed that corporate profit margins, then at record highs of 10% versus 
a historical average of 6%, would inevitably revert to the mean. Year 
after year—2012, 2013, 2014, 2015, 2016, 2017—the investor waited for this reversion, holding 
cash while the stock market more than doubled.

The investor's initial belief wasn't unreasonable given historical data. The real 
mistake was **failing to update those beliefs** efficiently as disconfirming 
evidence accumulated. Each year of sustained high profit margins was information 
that should have shifted the investor's stance, but didn't.

Winning in the investing game—and in life—isn't simply about having correct 
prior beliefs. It's about **efficiently updating those beliefs in response to feedback from reality**.

This project explores these ideas through a coin-flipping game that 
serves as a microcosm of Bayesian learning. You will:
...
- A green coin flip is worth: $0.70 \times 2.00 + 0.30 \times 0.00 = 1.40$ (in €)
- A red coin flip is worth: $0.30 \times 2.00 + 0.70 \times 0.00 = 0.60$ (in €)
