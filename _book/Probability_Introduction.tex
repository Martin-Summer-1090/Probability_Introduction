...
the state space as \({\cal S}=\{u,m,d\}\). It is however impossible to determine the probability
for each of these three elements. It is also not possible to determine the probability
...
In the example of tossing a fair coin twice we found that the state space is
...
the probability as a classical probability. We could also give this postulate a frequency
interpretation using our knowledge of R.

We proceed by analogy to our approach of rolling a die. This time, however the
outcome is a character not a number. R understands characters. A character vector in R
stores small pieces of text. You can create a character vector in R by surrounding the string
of characters by a quote, either as " " or as ' '. Both notations are valid. Let us
...
This looks already pretty good. Lets toss even more. Why not a million of times?
...
If you have correctly figured out the state space, you will surely be able to which of the
following sequences is more likely i.e.~has the highes probability: \texttt{bbbb}, \texttt{bgbg} or
\texttt{gggg}?
...
example, we see that the probability of each sequence should be equal to \(1/16\). In reality
...
Let us come to our last example, the birthday problem, after we have introduced a last
new probability concept in this lecture. But before we do that let us introduce two
more examples illustrating the interpretation of subjective probability.
...
your idea. There is no way you could use classical or relative frequency probability.
...
will roll a 5 and then a 6? Note that this is a question, slightly different from what
