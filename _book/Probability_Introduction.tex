...
\tightlist
\item[Complement]
The \textbf{complement} of an event \(A\) with respect to an event \(B\) is the set of all elements that are in \(B\) but not in \(A\). Set complementation is written as \(A \setminus B\).
\end{description}
...
Two events \(A\) and \(B\) are said to be \textbf{mutually exclusive} if they have no basic outcomes in common. The
