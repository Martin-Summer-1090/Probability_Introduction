...
To close this lecture let us come back to proabability and discuss some more examples of
random experiments, sample spaces, basic outcomes and events. Let us try to leverage our
R knowlegde to build the objects in the different examples ourselves.
...
we can thing of this as a random experiment. Can you figure out the sample space ? Can you figure out
...
in probability: The random placement of \texttt{r\textasciigrave{}\textasciigrave{}\ balls\ in}n\texttt{cells.\ The\ mathematical\ notation\ for\ such\ a\ counting\ problem\ is:\ \$\textbackslash{}binom\{n\}\{r\}\ =\ \textbackslash{}frac\{n!\}\{k!(n\ -\ r)!\}\$\ The\ left\ habd\ side\ of\ the\ equation\ means\ "the\ number\ of\ ways\ to\ choose\ \$r\$\ things\ from\ among\ \$n\$\ things.\ The\ right\ hand\ side\ shows\ a\ formular\ how\ this\ can\ be\ computed.\ R\ has\ a\ function\ to\ compute\ these\ things\ directly.\ It\ is\ called}choose()` and we apply it to the birthdays example
...
can finally proceed to introduce the notion of \emph{probability}. From the preceeding lecture it should be
...
A sample spac is called \emph{discrete} if it contains only finitely many points, or infinitely many
...
\caption{The meaning of set union}\label{fig:unnamed-chunk-9}
\end{figure}
...
\caption{The meaning of set intersection}\label{fig:unnamed-chunk-12}
\end{figure}
...
The R function for computing intersections is calles \texttt{intersect()}. We call this function for
our example
...
\caption{The meaning of set intersection}\label{fig:unnamed-chunk-14}
\end{figure}
