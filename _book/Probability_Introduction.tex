...
have the same birthday. We will talk about probabilities in the next lecture. But in
this lecture we have learned that we can only talk meaningfully about probabilities if
we know the random experiment and the basic outcomes.

Again we can conceptually think of the birthday
of the students in the class as the outcome of a random experiment. Let us assume for
simplicity that the year has 356 days ignoring leap years. If we have \(n\) students
the number of ways these students can have birthdays in 365 day long year is
\(365^n\). This is the sample space \({\cal S}\) consists of all these different birthday
profiles. With 10 students we have \(4.1969e+25\) such profiles.

The event we are interested in is that \emph{at least two students have the same birthday}. It turns
out that it is easier to think about the complementary event, which is that
\emph{all students have different birthdays}. The way that \(n\) students can have different birthdays
is \(365 \cdot 364 \cdot 363 \ldots 365 - n + 1\). So for instance for 10 students this would
amount to

\begin{Shaded}
\begin{Highlighting}[]
\DecValTok{365}\SpecialCharTok{*}\DecValTok{364}\SpecialCharTok{*}\DecValTok{363}\SpecialCharTok{*}\DecValTok{362}\SpecialCharTok{*}\DecValTok{361}\SpecialCharTok{*}\DecValTok{360}\SpecialCharTok{*}\DecValTok{359}\SpecialCharTok{*}\DecValTok{358}\SpecialCharTok{*}\DecValTok{357}\SpecialCharTok{*}\DecValTok{356}
\end{Highlighting}
\end{Shaded}
...
We can by the way use R functions to calculate this big product without typing the different
numbers so many times. We can use the product function to to this:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{prod}\NormalTok{(}\DecValTok{365}\SpecialCharTok{:}\NormalTok{(}\DecValTok{365{-}10}\SpecialCharTok{+}\DecValTok{1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{Probability_Introduction_files/figure-latex/unnamed-chunk-24-1.pdf}
...
\hypertarget{birthdays-and-blockchains}{%
\section{Birthdays and Blockchains}\label{birthdays-and-blockchains}}

Let us go back to the problem we discussed in lecture 1 where we had \(n\) people in a room. How
much would you bet that there is nobody in the room who shares the same birthday?

We have worked out the numbers in lecture 1. We want to know \(P(\text{all different birthdays}\).
...
\hypertarget{baysian-updating-stock-market-and-overconfidence}{%
\section{Baysian Updating, Stock Market and Overconfidence}\label{baysian-updating-stock-market-and-overconfidence}}

\hypertarget{r-concepts}{%
\section{R-Concepts}\label{r-concepts}}
