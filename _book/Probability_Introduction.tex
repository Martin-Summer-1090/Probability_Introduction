...
outcomes are all the possible configurations of 10 birthdays among the 365 days of the
year.

Here we encounter a situation where it would be possible but impractical to write down the sample
space explicitly. It would just be a very long list of 10 dates long sequences of
...
Let us go back to the problem we discussed in lecture 1 where we had 10 people in a room. How
much would you bet that there is nobody in the room who shares the same birthday?

Let us think about this problem a bit more generally. Say the number of people in the room is
some number \(n\). Let \(A\) denote the event that at least two people in the room share the
same birthday. We denote the probability of \(A\) by \(P(A)\). To calculate this
probability it is easiest to realize that the probability that at least two people share the
same birthday is equivalent to the 1 minus the probability that nobody shares the same
birthday, which is the complementary event \(A^C\). For \(n\) people in the room there are in
total \(365^n\) possible outcomes and the event \(A\) can happen in \(365 \cdot 364 \cdot 363 \ldots (365-n+1)\) ways. Thus we get
\(P(A) = 1 - P(A^C) = 1 - \frac{365 \cdot 364 \cdot 363 \ldots (365-n+1)}{365^n}\)
It can be shown that this expression is equal to
\(\frac{365 \, !}{365 \, ! (365 - n) \, !}\)
