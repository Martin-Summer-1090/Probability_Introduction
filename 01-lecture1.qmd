...
```{r chatGPT-prompt, out.width='90%', fig.align='center', fig.cap='Figure 2: The ChatGPT prompt', fig.label='Promptgpt', echo=FALSE}
...
```{r, scriptscreen, out.width='90%', fig.align='center', fig.cap='The quarto pop up window', echo = F}
...
```{r, scriptscreen, out.width='90%', fig.align='center', fig.cap='Combining text and code in quarto documents', echo = F}
...
5. **Adding Comments and Output**:
   You can interweave your code with text explanations to make the document self-contained and easy to understand. For example:

```{r, scriptscreen, out.width='90%', fig.align='center', fig.cap='Combining text and code in quarto documents', echo = F}
knitr::include_graphics('figures/code_and_text.png')
...
files that are both functional and easy to share with others. 
...
concepts that we did not yet discuss. But we will learn them soon. For
the moment don't worry about these details and just see what this code 
does:
...
but there is still quite some variation. 
