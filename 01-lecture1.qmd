...
underlying probabilities by visualizing the frequency polts of outcomes?
...
counting equally probably cases -  to 
...
Let $A$ denote the event that no two people share the same brithday.
...
Base R containts a lot of powerful plotting functions which work
independently of `gglot2`.]
...
\end{equation*}
Simplify the summation by usin the fact that 
$\sum_{k=0}^{n-1} = \frac{n(n-1)}{2}$ we get
\begin{equation*}
